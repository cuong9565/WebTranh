...
// Nút "Áp dụng" reload lại trang index
...
document.addEventListener('click', (e) => {
    if (!btnCategory.contains(e.target) && !categoryOptions.contains(e.target)) {
...
btnCategory.addEventListener('click', () => {
    categoryOptions.style.display = categoryOptions.style.display === 'flex' ? 'none' : 'flex';
});

// Chọn/bỏ chọn danh mục khi click
categoryItems.forEach(item => {
    item.addEventListener('click', () => {
        ...
        console.log('Danh mục chọn:', selectedCategories);
    });
});
